...
function renderClubs(list) {
    ...
    container.innerHTML = list.map(club => {
        ...
        const deleteButtonHTML = isAdmin
            ? `<button onclick="deleteClub(\`${club.name}\`)" style="background: #e31837; color: white; padding: 5px 10px; border: none; cursor: pointer; margin-top: 10px; border-radius: 4px; font-size: 0.8rem;">Delete Club</button>`
            : '';
...
function setupAdminFeatures() {
    ...
    adminSection.innerHTML = `
        ...
        <div style="display: flex; gap: 10px; margin-top: 10px; flex-wrap: wrap;">
            ...
            <button onclick="addNewClub()" style="background: #e31837; color: white; padding: 8px 16px; border: none; cursor: pointer;">Add Club</button>
